...
("L'Étranger", "Albert Camus", 1942, "Roman", TRUE),
("1984", "George Orwell", 1949, "Dystopie", TRUE),
("Le Petit Prince", "Antoine de Saint-Exupéry", 1943, "Conte", FALSE),
("Les Misérables", "Victor Hugo", 1862, "Roman", TRUE),
...
("L'Alchimiste", "Paulo Coelho", 1988, "Philosophique", TRUE);
...
SELECT * FROM livres WHERE annee_parution>2000;
...
SELECT * FROM livres ORDER BY annee_parution DESC;

SELECT * FROM livres WHERE LOWER(titre) LIKE "%le%";

SELECT COUNT(titre) FROM livres WHERE disponible=TRUE ;

SELECT * FROM livres ORDER BY annee_parution DESC LIMIT 1;
SELECT * FROM livres ORDER BY annee_parution ASC LIMIT 1;



SELECT * FROM livres ORDER BY titre LIMIT 3;
-- S-Challenge 3 — Ma première jointure

--  Crée une deuxième table `emprunts` avec : id, livre_id (clé étrangère vers livres),
--   nom_emprunteur, date_emprunt, date_retour_prevue, retourne (booléen).
--    Insère 6 emprunts (certains retournés, certains en cours).
--     Écris une jointure pour afficher le nom du livre ET le nom de l'emprunteur pour chaque emprunt. 
--     Filtre pour n'afficher que les emprunts non retournés.

-- ---
CREATE TABLE emprunts(
    id INT PRIMARY KEY AUTO_INCREMENT,
    nom_emprunteur VARCHAR(70),
    date_emprunt DATE,
...
INSERT INTO emprunts (nom_emprunteur, date_emprunt, date_retour_prevue, retourne, livre_id) VALUES
("Ali", "2024-03-01", "2024-03-10", TRUE, 1),
("Sara", "2024-03-05", "2024-03-15", FALSE, 2),
("Omar", "2024-03-07", "2024-03-17", TRUE, 3),
("Nora", "2024-03-10", "2024-03-20", FALSE, 4),
("Yassine", "2024-03-12", "2024-03-22", TRUE, 5),
("Lina", "2024-03-15", "2024-03-25", FALSE, 6);


SELECT livres.titre ,emprunts.nom_emprunteur 
FROM livres
INNER JOIN emprunts
on emprunts.livre_id=livres.id


SELECT * FROM emprunts WHERE retourne=false;

-- ## S-Challenge 4 — Lire un diagramme de cas d'utilisation

-- Observe le texte suivant et dessine le diagramme de cas d'utilisation correspondant sur Draw.io :
--  Une application de cinéma permet aux visiteurs de consulter les films à l'affiche et les horaires.
--  Les membres (visiteurs connectés) peuvent en plus réserver des places et noter les films. Le gérant peut ajouter des films,
--   modifier les horaires et consulter les réservations. Le système envoie automatiquement des confirmations de réservation par email.

-- ---

-- ## S-Challenge 5 — Lire un diagramme de classes

--  À partir de cette description, dessine le diagramme de classes : Une école a des étudiants et des professeurs.
--   Les étudiants sont inscrits à des cours. Chaque cours est animé par un professeur. Un cours a un intitulé, 
--   un nombre d'heures et un coefficient. Un étudiant a un numéro, un nom et peut passer des examens.
--    Un examen est lié à un cours, a une date et une note.
...
-- Tu modélises le système d'une plateforme comme ONCF. Identifie les acteurs (voyageur, agent ONCF,
--  administrateur, système de paiement)
--  et modélise au minimum 12 cas d'utilisation. Utilise des relations `include` pour les
--   comportements obligatoires partagés (ex: "Consulter les horaires" qui est inclus dans "Réserver un billet")
--    et des relations `extend` pour les comportements optionnels (ex: "Sélectionner un siège" qui étend "Réserver un billet").
--     Rédige le scénario nominal et le scénario d'erreur pour le cas "Réserver un billet".

-- ---

-- ## R-Challenge 2 — Le système d'information hospitalier

-- -- Tu modélises la partie rendez-vous et dossiers patients d'un hôpital. Les acteurs sont : patient, médecin, infirmier, réceptionniste,
--  système de rappel automatique. Identifie au moins 15 cas d'utilisation couvrant la prise de rendez-vous,
--   la gestion du dossier médical, la prescription et la facturation. Identifie précisément 3 relations `include`
--    et 3 relations `extend` que tu justifies dans un commentaire sur le diagramme.

-- ---

-- ## R-Challenge 3 — L'application de covoiturage 🔥

-- -- Tu modélises une application type BlaBlaCar. Les acteurs sont : conducteur, passager (un utilisateur peut être les deux),
--  administrateur, système de géolocalisation, service de paiement. Identifie au minimum 14 cas d'utilisation. 
--  Un conducteur peut publier un trajet, gérer ses réservations et évaluer les passagers.
--   Un passager peut rechercher un trajet avec filtres, réserver, payer et évaluer le conducteur. 
--   L'administrateur peut modérer les avis et gérer les litiges. Justifie par écrit pourquoi 
--   "Vérifier le permis de conduire" est en `include` de "Publier un trajet" et non en `extend`.
...
-- Tu travailles sur la base de données d'une chaîne de restaurants avec les tables :
--  restaurants (id, nom, ville, date_ouverture), employes (id, restaurant_id, nom, poste, salaire), 
--  menus (id, restaurant_id, nom_plat, prix, categorie), clients (id, nom, email, ville),
--   commandes (id, client_id, restaurant_id, date_commande, montant_total),
--    lignes_commande (id, commande_id, menu_id, quantite, prix_unitaire).
...
CREATE TABLE lignes_commande(
    id_lignes_commande INT PRIMARY KEY AUTO_INCREMENT,
    quantite FLOAT,
    prix_unitaire FLOAT,
    commande_id INT,
...
-- Écris les requêtes pour : le chiffre d'affaires par restaurant ce mois-ci,
--  la masse salariale par restaurant, les 5 plats les plus commandés toutes restaurants confondues,
--   les clients qui ont commandé dans plus d'un restaurant, le panier moyen par ville de restaurant,
--    les restaurants dont le CA est supérieur à la moyenne.
