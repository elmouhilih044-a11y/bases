...
function filtrerParStatut(commandes, statut) {
  const result=commandes.filter(commande=>commande.statut==statut);
  return result;
  
}

function calculerChiffreAffaires(commandes) {
  const som=commandes.reduce((acc,n)=>acc+n.montant,0);
  return som;
}

function commandeLaPlusElevee(commandes) {
  return commandes.reduce((a,b)=>a.montant>b.montant?a:b);
}

function marquerPrioritaires(commandes) {
  commandes.forEach(c=>{
  if(c.montant>800 && c.statut==='en_attente'){
     c.prioritaire=true;
  }
  else{
   c.prioritaire=false;
...
function resumeParStatut(commandes) {
  return commandes.reduce((acc, cmd) => {
    ...
    if (!acc[cmd.statut]) {
      ...
      acc[cmd.statut].count += 1;
      acc[cmd.statut].total += cmd.montant;
    }
