...
class Voiture
{
    ...
    public function setAnnee(int $annee)
    {
        ...
        if ($annee < 1886 || $annee > $anneeActuel) {
    ...
    public function setKilometrage($kilometrage)
    {
        ...
        if ($this->kilometrage !== null && $this->kilometrage > $kilometrage) {
            throw new Exception("Impossible de diminuer le kilométrage");
        }
        $this->kilometrage = $kilometrage;
    }
...
return $this->kilometrage;
    ...
    public function calculerAge(){
        $age=(int)date("Y")-(int)$this->annee;
        return $age;
    }

    public function EstVintage(){
         $age=(int)date("Y")-(int)$this->annee;
        if($age>25){
            return true;
        }
...
public function estimerValeur(): float
{
    $valeur = 200000;
    $age = (int) date("Y") - $this->annee;

    $valeur -= $valeur * 0.1 * $age;

   
    $tranches = $this->kilometrage / 10000;


    $valeur -= 200000 * 0.005 * $tranches;
...
$v1 = new Voiture("BMW", "X5", 2022, "Noire", 30000);

echo $v1->description() . "\n";
echo $v1->setAnnee(2001). "\n";
echo $v1->description() . "\n";
echo $v1->calculerAge(). "\n";
echo $v1->estVintage(). "\n";
...
public function retirerVoiture($immatriculation) {
    foreach ($this->voitures as $key => $v) {

        if ($v->immatriculation == $immatriculation){
            unset($this->voitures[$key]);
            return;
        }
...
// Crée une classe `Animal` avec les propriétés nom, espece, age et une méthode `sePresenter()`.
//  Crée une classe `Chien` qui hérite d'Animal et ajoute la propriété race et une méthode `aboyer()`. 
//  Crée une classe `Chat` qui hérite d'Animal et ajoute la propriété couleur et une méthode `ronronner()`. 
//  Dans Chien et Chat, surcharge `sePresenter()` pour inclure les informations spécifiques.
